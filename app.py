...
def inject_cmf_styles(login: bool = False) -> None:
    ...
    .stApp {{
        background:
            radial-gradient(circle at 85% 15%, rgba(139,61,255,0.24) 0%, rgba(139,61,255,0.00) 34%),
            radial-gradient(circle at 10% 0%, rgba(88,199,243,0.14) 0%, rgba(88,199,243,0.00) 30%),
            linear-gradient(180deg, #041F5F 0%, #061946 100%),
            {bg_image};
    ...
    .cmf-hero {{
        background: linear-gradient(110deg, rgba(8,27,69,0.96) 0%, rgba(8,43,116,0.92) 55%, rgba(139,61,255,0.24) 100%);
...
def render_login() -> None:
    ...
    st.markdown(
        f"""
        <div class="cmf-login-wrapper">
            <div class="cmf-login-card">
                {logo_html}
                <div class="cmf-login-title">Bienvenido</div>
                <div class="cmf-login-subtitle">Monitor AFP · Cartera de Fondos de Pensiones</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    # Centramos el input usando columnas Streamlit para que funcione bien en todos los navegadores.
    left, center, right = st.columns([1.05, 1, 1.05])
    with center:
        password = st.text_input("Clave de acceso", type="password", label_visibility="collapsed", placeholder="Ingresa tu clave")
...
def render_cmf_header() -> None:
    ...
    st.markdown(
        ...
        <div class="cmf-hero">
            {logo_html}
            <div class="cmf-hero-title">AFP SP · Cartera de Fondos de Pensiones</div>
            <div class="cmf-hero-subtitle">
                Monitor de composición de cartera, fondos, contrapartes bancarias y AUM usando bases públicas de la Superintendencia de Pensiones.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
